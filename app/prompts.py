...
class Prompts:
    summarize_file_diff = """## GitHub PR Title
...
"""
    ...
    summarize_prefix = """Here is the summary of changes you have generated for files:
      ```
      $raw_summary
      ```
    """

    summarize_short = """Your task is to provide a concise summary of the changes. This 
...
- The summary should not exceed 500 words."""
    ...
    review_file_diff = """## GitHub PR Title
...
Output: Review comments in markdown with exact line number ranges in new hunks. Start and end line numbers must be 
within the same hunk. For single-line comments, start=end line number. Must use example response format below.
Use fenced code blocks using the relevant language identifier where applicable.
Don't annotate code snippets with line numbers. Format and indent code correctly.
Do not use `suggestion` code blocks.
For fixes, use `diff` code blocks, marking changes with `+` or `-`. The line number range for comments with fix snippets 
must exactly match the range to replace in the new hunk.
...
```"""
    ...
    def render_summarize_file_diff(self, inputs: Inputs, review_simple_changes: bool) -> str:
